...
from db_interactions import insert_data, delete_data
...
class TestDBInteractions(TestCase):
    ...
    def test_insert_data_riders(self):
        ...
        add_ = insert_data(self.session, 'Riders', **data_to_add)
    ...
    def test_insert_data_trackers(self):
        ...
        self.assertTrue(insert_data(self.session, 'PhysicalLocations', **physical_location))
        self.assertTrue(insert_data(self.session, 'Trackers', **tracker))
    ...
    def test_insert_data_TrackerLocations_physical(self):
        ...
        add_ = insert_data(self.session, 'PhysicalLocations', **data_to_add)
        self.assertTrue(add_)
        data_in_db = self.cur.execute('SELECT * FROM tracker_locations;').fetchall()[0]
    ...
    def test_add_data_trackerlocations_personal(self):
        ...
        add_ = insert_data(self.session, 'PersonalLocations', **data_to_add)
        self.assertTrue(add_)
        data_in_db = self.cur.execute('SELECT * FROM tracker_locations;').fetchall()[0]
    ...
    def test_delete_data_riders(self):
        ...
        self.conn.close()
        delete_ = delete_data(self.session, 'Riders', 1)
    ...
    def test_delete_nonexistentdata(self):
        # no data in db
        delete_ = delete_data(self.session, 'PhysicalLocations', 1)
